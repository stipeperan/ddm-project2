...
\begin{document}
    ...
    As explained in the \nameref{sec:proj-scen} introduction, we began with the original ER model and the previous Neo4J structure, then adapted and refined several attributes to better suit MongoDB's document-oriented design.
    For instance, we chose not to create a separate \texttt{Form} collection. Instead, we embedded that information directly within each Pokémon document, where it fits more naturally.
    As shown in Figure~\ref{fig:mongo-structure}, the final result of this transformation can be clearly observed in the structure of the MongoDB database. 
The \texttt{mixed} field type indicates that a value may be either \texttt{null} or \texttt{string}, reflecting optional attributes within the dataset.
    ...
    \begin{itemize}
    \item \textbf{Pokemon}: canonical Pokémon entries with attributes, types, forms and evolutions.
    \item \textbf{Trainer}: documents describing each trainer, the Pokémon they own and which gym they lead.
